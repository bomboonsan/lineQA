...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://boschthailandbackend.bomboonsan.com/contest`);
          ...
          {data.map((value, index) => (
            <Table.Row key={index}>
              <Table.Cell>
                ...
                <Link href={`https://liff.line.me/1661407578-X6ro31ow#idcontest${value._id}`} target="_blank" className='hover:text-blue-600'>
                  {value.title}
                </Link>
              </Table.Cell>
              <Table.Cell>
              <Button size="xs" flat data-url={`https://liff.line.me/1661407578-X6ro31ow#idcontest${value._id}`} onClick={(e) => handleCopyUrl(e)}>GET LINK</Button>
              </Table.Cell>   
              <Table.Cell>
                {value.campaign}
              </Table.Cell>
